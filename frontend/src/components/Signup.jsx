...
function Signup (props) {
  ...
  <form method='POST' className='grid grid-cols-2 gap-4 text-white'>
      ...
      <label htmlFor="username">Username</label><input id="username" className={css} onChange={(evt) => setUsername(evt.target.value)} />
      <label htmlFor="email">Email</label><input id="email" className={css} onChange={(evt) => setEmail(evt.target.value)} />
      ...
      <button type='submit' className='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded' onClick={submissionHandler}>Sign Up</button>
